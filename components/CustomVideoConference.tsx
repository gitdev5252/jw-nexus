...
export default function CustomVideoConference() {
  ...
  const initialStart = typeof window !== 'undefined' ? localStorage.getItem('meetingStart') : null;
  ...
  const localTrack = localParticipant?.getTrackPublication(Track.Source.Camera)?.track;
  const toggleHand = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white p-2 sm:p-4 text-black relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2 sm:gap-0">
        <div className="text-left text-lg font-medium py-2">Friends Catch Up Call</div>
              ...
              .map((trackRef) => {
                ...
                return (
                  <div key={trackRef.publication?.trackSid} className="relative w-full h-full">
                    <ScreenShareVideo track={track ?? null} />
                  ...
                  >
                    {track ? (
                      ...
                      <div className="flex flex-col items-center justify-center w-full h-full">
                        ...
                        <span className="mt-2 text-gray-700 font-medium">{p.name || 'Guest'}</span>
                        <span className="text-xs text-gray-500">Camera is off</span>
                      </div>
                    )}
                  </div>
      ...
      <div className="flex items-center justify-center sm:justify-end px-2 sm:px-10 py-3">
        <div className="flex flex-wrap gap-6 sm:gap-10 justify-center">
          ...
          <ControlIcon
            ...
            onClick={toggleHand}
          />
          <ControlIcon icon={<Smile size={20} />} label="Emoji" onClick={sendEmoji} />
...
function ControlIcon({
  ...
  return (
    ...
    >
      <div className={`${bgColor} ${hoverColor} p-3 rounded-xl shadow-md`}>{icon}</div>
